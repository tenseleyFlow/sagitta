...
typedef enum { YEW_HUNK_ADD, YEW_HUNK_DEL, YEW_HUNK_MOD } HunkKind;
typedef struct GitHunk { LineNo base_lo, base_n; LineNo buf_lo, buf_n; HunkKind kind; } GitHunk;
VEC_DECL(GitHunkVec, GitHunk);
typedef struct HunkList { Arena a; GitHunkVec h; u64 buf_gen; char base_oid[65]; bool base_is_head; bool truncated; } HunkList;
