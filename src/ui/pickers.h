#ifndef YEW_UI_PICKERS_H
#define YEW_UI_PICKERS_H
 ...
 * from yew_undo_describe with `now` PASSED IN rather than read from a
 * clock, so "3 minutes ago" is deterministic and the goldens can pin it
 * (invariant 5).  yew_undo_dump stays as the headless surface.
 */
...
/* Releases the shared item table; called from yew_ed_free. */
