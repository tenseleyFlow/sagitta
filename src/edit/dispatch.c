...
static void dispatch_set_seq_message(Ed *ed, const char *prefix)
{
    ...
    yew_key_format_seq(ed->chord.seq, ed->chord.n, &text);
    len = text.len;
    if (len >= sizeof(ed->dispatch_message))
        len = sizeof(ed->dispatch_message) - 1U;
    if (len != 0U)
...
{
    const char *value = getenv("YEW_CHORD_TIMEOUT_MS");
    char *end = NULL;
    unsigned long parsed;

    if (value == NULL || *value == '\0')
        return YEW_CHORD_TIMEOUT_DEFAULT_MS;
    errno = 0;
                                       ...
                                       const Binding **binding)
{
    ...
    return yew_keymap_lookup(owner, ed->chord.seq, ed->chord.n, NULL,
                             binding);
}
...
static void dispatch_fire(Ed *ed, const Binding *selected)
{
    ...
    if (desc != NULL && (desc->flags & YEW_CMD_CAPTURES_TEXT) != 0U &&
        binding.sarg == NULL) {
    ...
    cx = (CmdCtx){0};
    cx.ed = ed;
    cx.win = ed->cmdline.active ? yew_cmdline_target(ed) : ed->win;
    ...
    ed->last_status = yew_ed_invoke(ed, binding.cmd, &cx);
    ed->dispatch_count++;
}
...
static bool dispatch_take_count(Ed *ed, KeyId key)
{
    ...
    if (ed->mode >= YEW_MODE__N || !yew_modes[ed->mode].takes_count ||
    ...
    if (ed->chord.count > (YEW_COUNT_MAX - digit) / 10U) {
        ed->chord.count = YEW_COUNT_MAX;
...
void yew_dispatch_init(Ed *ed)
{
    ...
    if (ed == NULL)
        YEW_BUG("dispatch init: NULL editor");
    ...
    ed->last_status = YEW_CMD_OK;
    dispatch_reset_capture(ed);
    yew_keys_default_install(ed);
}
...
void yew_dispatch_free(Ed *ed)
{
    ...
    for (i = 0U; i < YEW_MODE__N; i++)
        yew_keymap_free(&ed->mode_keys[i]);
    for (i = 0U; i < YEW_MODE__N; i++)
        yew_keymap_free(&ed->bind_keys[i]);
    yew_keymap_free(&ed->user_keys);
    (void)memset(&ed->keys, 0, sizeof(ed->keys));
    dispatch_reset_chord(ed);
    dispatch_reset_capture(ed);
    ed->dispatch_message[0] = '\0';
...
void yew_dispatch_key(Ed *ed, Key key, i64 now_ms)
{
    ...
    if (ed->tab_prompt.active) {
        u8 answer = key.code == YEW_KEY_ESCAPE
                    ? 0x1BU
                    : (key.ntext == 1U ? key.text[0] : 0U);

        if (yew_tab_prompt_key(ed, answer))
            return;
    }
    ...
    if (ed->ws_prompt.active) {
        ...
        if (yew_ws_prompt_key(ed, answer))
            return;
    }
    if (ed->confirm.active) {
        ...
        if (yew_search_confirm_key(ed, answer))
            return;
    }
    if (ed->capture_cmd.v != 0U) {
        ...
        if (key.ntext == 0U) {
            ...
            ed->last_cmd = cmd;
            ed->last_status = YEW_CMD_ERR_ARG;
            ed->dispatch_count++;
            return;
        }
        ed->dispatch_message[0] = '\0';
        ed->last_cmd = cmd;
        ed->last_status = yew_ed_invoke(ed, cmd, &cx);
        ed->dispatch_count++;
        return;
    }
    id = yew_keyid(key);
    if ((u32)(id.v >> 16U) == YEW_KEY_ESCAPE &&
    ...
    if (ed->chord.n == 0U) {
        ...
        match = yew_keystack_lookup(&ed->keys, seq, 1U, &layer, &node,
                                    &binding);
    ...
    if (match == YEW_MATCH_PREFIX || match == YEW_MATCH_FULL_PREFIX) {
        dispatch_arm(ed, now_ms);
        return;
    }
...
{
    if (mode >= YEW_MODE__N)
        YEW_BUG("invalid editor mode %u", (u32)mode);
    dispatch_reset_chord(ed);
    dispatch_reset_capture(ed);
    ed->mode = mode;
