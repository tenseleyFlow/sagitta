                        ...
                        const char **err)
{
    ...
    for (i = 0U; i < len; i++) {
        switch (s[i]) {
        case 'g': *flags |= YEW_SUB_GLOBAL; break;
        case 'c': *flags |= YEW_SUB_CONFIRM; break;
        case 'n': *flags |= YEW_SUB_COUNT_ONLY; break;
        case 'i': *flags |= YEW_SUB_ICASE; break;
        case 'I': *flags |= YEW_SUB_CASE; break;
        case 'p': *flags |= YEW_SUB_PRESERVE; break;
        case 'e': *flags |= YEW_SUB_QUIET; break;
        case ' ': case '\t': break;
        default:
            *err = "unknown :s flag";
            return false;
    ...
    if ((*flags & YEW_SUB_ICASE) != 0U && (*flags & YEW_SUB_CASE) != 0U) {
...
static bool sub_range(CmdCtx *cx, LineNo *lo, LineNo *hi)
{
    ...
    case YEW_RANGE_LINES:
        ...
        *hi = cx->range.hi;
        return true;
    default: {
        const Cursor *c = yew_ed_cursor(cx->ed);

        if (c == NULL)
            return false;
        *lo = yew_textbuf_line_of(tb, c->pos);
        *hi = *lo;
        return true;
    }
    }
...
CmdStatus yew_search_cmd_replace(CmdCtx *cx)
{
    SubParts parts;
    const char *err = NULL;
    SearchOpts opts;
    ...
    if ((parts.flags & YEW_SUB_ICASE) != 0U) {
        opts.ignorecase = true;
        opts.smartcase = false;
    } else if ((parts.flags & YEW_SUB_CASE) != 0U) {
     ...
     */
    if (parts.patlen == 0U) {
        const RegVal *slash = yew_reg_get(&cx->ed->regs, (u8)'/');

        if (slash == NULL || slash->bytes.len == 0U) {
            yew_msg(cx->ed, YEW_MSG_ERROR, "no previous search pattern");
    ...
    re = yew_search_compile(&arena, parts.pat, parts.patlen, &opts, &rerr);
    if (re == NULL) {
        yew_msg(cx->ed, YEW_MSG_ERROR, "%s (at %u)",
    ...
    if (!yew_repl_check(parts.rep, parts.replen, yew_re_group_count(re),
                        &terr)) {
        yew_msg(cx->ed, YEW_MSG_ERROR, "%s (at %u)",
    ...
    if (!yew_repl_plan_build(&plan, re, cx->win->buf->tb, lo, hi,
    ...
    if (plan.len == 0U) {
        ...
        if ((parts.flags & YEW_SUB_QUIET) == 0U) {
            Bytebuf shown;

            bytebuf_init(&shown);
            bytebuf_append(&shown, parts.pat, parts.patlen);
            yew_msg(cx->ed, YEW_MSG_ERROR, "pattern not found: %.*s",
                    (int)shown.len, (const char *)shown.data);
            bytebuf_free(&shown);
            status = YEW_CMD_ERR_STATE;
        }
        yew_repl_plan_free(&plan);
        arena_free_all(&arena);
        return status;
    }
    if ((parts.flags & YEW_SUB_COUNT_ONLY) != 0U) {
        yew_msg(cx->ed, YEW_MSG_INFO, "%u matches on %u lines",
                (unsigned)plan.len, (unsigned)plan.lines);
        yew_repl_plan_free(&plan);
        arena_free_all(&arena);
        return YEW_CMD_OK;
    }
    if ((parts.flags & YEW_SUB_CONFIRM) != 0U) {
        /*
         ...
         */
        yew_search_confirm_start(cx->ed, &plan, parts.rep, parts.replen);
        arena_free_all(&arena);
        return YEW_CMD_OK;
    }
    {
        EditCtx ec = yew_ed_edit_ctx(cx->ed);
        u32 n = yew_repl_plan_apply(&plan, &ec);

        yew_ed_finish_edit(cx->ed, &ec);
        yew_msg(cx->ed, YEW_MSG_INFO, "%u replacements on %u lines",
...
{
    const YewReplEdit *cur = yew_repl_confirm_current(&ed->confirm.walk);
    const Cursor *c;
    ...
    c = yew_ed_cursor(ed);
    if (c != NULL)
        yew_ed_cursor(ed)->pos = BYTEOFF(cur->span.lo);
    yew_win_follow_cursor(ed->win);
    yew_ed_damage_document(ed);
    yew_msg(ed, YEW_MSG_INFO, "replace with '%.*s'? (y/n/a/q/l/^E/^Y)",
...
static void confirm_finish(Ed *ed)
{
     ...
     */
    n = yew_repl_plan_apply(&ed->confirm.plan, &ec);
    yew_ed_finish_edit(ed, &ec);
    yew_msg(ed, YEW_MSG_INFO, "%u replacements on %u lines", (unsigned)n,
            (unsigned)ed->confirm.plan.lines);
    yew_repl_plan_free(&ed->confirm.plan);
    bytebuf_free(&ed->confirm.shown);
    ed->confirm.active = false;
    yew_ed_damage_document(ed);
}

void yew_search_confirm_start(Ed *ed, YewReplPlan *plan, const char *rep,
                              size_t replen)
{
    ...
    yew_repl_confirm_begin(&ed->confirm.walk, &ed->confirm.plan);
    ed->confirm.active = true;
    if (!yew_repl_confirm_pending(&ed->confirm.walk)) {
        confirm_finish(ed);
        return;
    }
    confirm_prompt(ed);
...
bool yew_search_confirm_key(Ed *ed, u8 key)
{
    if (ed == NULL || !ed->confirm.active)
        return false;
    if (!yew_repl_confirm_answer(&ed->confirm.walk, key)) {
...
void yew_search_confirm_reprompt(Ed *ed)
{
    if (ed == NULL || !ed->confirm.active)
        return;
    if (yew_repl_confirm_pending(&ed->confirm.walk))
...
void yew_search_confirm_cancel(Ed *ed)
{
    if (ed == NULL || !ed->confirm.active)
        return;
    yew_repl_plan_free(&ed->confirm.plan);
...
CmdStatus yew_search_cmd_open(CmdCtx *cx)
{
    /* `:search pat` seeds and accepts in one step; `/` opens the
     * prompt.  Both go through the same state. */
    yew_search_open(cx->ed, cx->win, false);
    return YEW_CMD_OK;
}

CmdStatus yew_search_cmd_open_back(CmdCtx *cx)
{
    yew_search_open(cx->ed, cx->win, true);
    return YEW_CMD_OK;
}
...
CmdStatus yew_search_cmd_next(CmdCtx *cx)
{
    (void)yew_search_step(cx->ed, cx->win, true, step_count(cx));
    return YEW_CMD_OK;
}

CmdStatus yew_search_cmd_prev(CmdCtx *cx)
{
    (void)yew_search_step(cx->ed, cx->win, false, step_count(cx));
    return YEW_CMD_OK;
}

CmdStatus yew_search_cmd_word_next(CmdCtx *cx)
{
    (void)yew_search_word(cx->ed, cx->win, true);
    return YEW_CMD_OK;
}

CmdStatus yew_search_cmd_word_prev(CmdCtx *cx)
{
    (void)yew_search_word(cx->ed, cx->win, false);
    return YEW_CMD_OK;
}

CmdStatus yew_search_cmd_clear_highlight(CmdCtx *cx)
{
    yew_search_clear_highlight(cx->ed, cx->win);
    return YEW_CMD_OK;
}
...
CmdStatus yew_mark_cmd_set(CmdCtx *cx)
{
    ...
    if (cx->win == NULL || cx->win->buf == NULL)
        return YEW_CMD_ERR_STATE;
    c = yew_ed_cursor(cx->ed);
    if (c == NULL)
        return YEW_CMD_ERR_STATE;
    if (!yew_ed_mark_set(cx->ed, cx->win->buf, name, c->pos)) {
        yew_msg(cx->ed, YEW_MSG_ERROR,
...
CmdStatus yew_mark_cmd_jump(CmdCtx *cx)
{
    u8 name = mark_name_arg(cx);
    ByteOff at;
    Cursor *c;
    ...
    if (cx->win == NULL || cx->win->buf == NULL)
        return YEW_CMD_ERR_STATE;
    if (!yew_ed_mark_get(cx->ed, cx->win->buf, name, &at)) {
        yew_msg(cx->ed, YEW_MSG_ERROR, "mark not set");
        return YEW_CMD_ERR_STATE;
    }
    c = yew_ed_cursor(cx->ed);
    if (c == NULL)
