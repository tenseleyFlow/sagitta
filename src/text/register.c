...
{
    if (v == NULL)
        YEW_BUG("yew_regval_init: NULL value");
...
static void regval_copy_raw(RegVal *dst, const RegVal *src)
{
    ...
    for (i = 0U; i < src->rows.len; i++)
        YewRegRowVec_push(&dst->rows, src->rows.data[i]);
    dst->type = src->type;
...
void yew_regval_copy(RegVal *dst, const RegVal *src)
{
    if (dst == NULL || src == NULL)
        YEW_BUG("yew_regval_copy: NULL value");
...
static void append_text_range(Bytebuf *out, const TextBuf *tb, Span range)
{
    ...
    if (range.lo > range.hi || range.hi > yew_textbuf_len(tb))
        YEW_BUG("register: source span out of bounds");
    total = range.hi - range.lo;
    if (total == 0U)
        return;
    if (!yew_textiter_begin(&it, tb, BYTEOFF(range.lo)))
        YEW_BUG("register: cannot iterate source span");
    while (done < total) {
        const u8 *bytes;
        u64 len;
        u64 take;
                          ...
                          RegType type, const FileMeta *meta)
{
    ...
    if (out == NULL || tb == NULL)
        YEW_BUG("yew_regval_from_span: NULL argument");
    if (type == YEW_REG_BLOCKWISE)
        YEW_BUG("blockwise capture requires per-row geometry");
    regval_clear(out);
    out->type = (u8)type;
    append_text_range(&out->bytes, tb, range);
    if (type == YEW_REG_LINEWISE && !ends_in_lf(&out->bytes)) {
        if (meta == NULL)
            YEW_BUG("linewise register capture requires file metadata");
        yew_filemeta_eol_bytes(meta, &eol, &eol_len);
...
static void free_many(RegVal *values, size_t count)
{
    size_t i;
    for (i = 0U; i < count; i++)
        yew_regval_free(&values[i]);
}

void yew_reg_init(Registers *r)
{
    if (r == NULL)
        YEW_BUG("yew_reg_init: NULL register file");
    (void)memset(r, 0, sizeof(*r));
    init_many(r->named, YEW_ARRAY_LEN(r->named));
    yew_regval_init(&r->unnamed);
    init_many(r->numbered, YEW_ARRAY_LEN(r->numbered));
    yew_regval_init(&r->small_del);
    yew_regval_init(&r->last_insert);
    yew_regval_init(&r->search);
    yew_regval_init(&r->cmdline);
    yew_regval_init(&r->file);
    yew_regval_init(&r->alt_file);
    yew_regval_init(&r->system);
    init_many(r->ring, YEW_ARRAY_LEN(r->ring));
    r->ring_depth = YEW_KILL_RING_DEPTH_DEFAULT;
    r->ring_bytes_max = YEW_KILL_RING_BYTES_DEFAULT;
    r->clip_read_max = UINT64_C(64) * 1024U * 1024U;
    r->clipboard_sync = YEW_CLIP_SYNC_YANK;
}
...
void yew_reg_free(Registers *r)
{
    if (r == NULL)
        return;
    free_many(r->named, YEW_ARRAY_LEN(r->named));
    yew_regval_free(&r->unnamed);
    free_many(r->numbered, YEW_ARRAY_LEN(r->numbered));
    yew_regval_free(&r->small_del);
    yew_regval_free(&r->last_insert);
    yew_regval_free(&r->search);
    yew_regval_free(&r->cmdline);
    yew_regval_free(&r->file);
    yew_regval_free(&r->alt_file);
    yew_regval_free(&r->system);
    free_many(r->ring, YEW_ARRAY_LEN(r->ring));
    YewRegPasteSpanVec_free(&r->paste_spans);
    (void)memset(r, 0, sizeof(*r));
}
...
{
    if (r == NULL)
        YEW_BUG("yew_reg_bind_context: NULL register file");
    r->bound_undo = undo;
    r->bound_meta = meta;
}
...
RegVal *yew_reg_get(Registers *r, u8 name)
{
    ...
    case '.':
        ...
        if (r->bound_undo != NULL)
            (void)yew_undo_last_insert(r->bound_undo,
                                       &r->last_insert.bytes,
...
void yew_reg_set(Registers *r, u8 name, const RegVal *v)
{
     ...
     * short list.  Sprint 21 opened `/` via yew_reg_set_search; the rest
    ...
    dst = yew_reg_get(r, name);
    if (dst == NULL)
        YEW_BUG("unknown register name 0x%02x", (unsigned)name);
    reg_set_raw(dst, v);
}
...
{
    if (r == NULL || v == NULL)
        YEW_BUG("yew_reg_set_macro: NULL argument");
    if (name < (u8)'a' || name > (u8)'z')
        YEW_BUG("invalid macro register name 0x%02x", (unsigned)name);
    if (append)
...
void yew_reg_set_cmdline(Registers *r, const u8 *bytes, size_t len)
{
    if (r == NULL || (bytes == NULL && len != 0U))
        YEW_BUG("yew_reg_set_cmdline: NULL argument");
    regval_clear(&r->cmdline);
    bytebuf_append(&r->cmdline.bytes, bytes, len);
    r->cmdline.type = YEW_REG_CHARWISE;
...
{
    if (r == NULL || (bytes == NULL && len != 0U))
        YEW_BUG("yew_reg_set_search: NULL argument");
    regval_clear(&r->search);
    bytebuf_append(&r->search.bytes, bytes, len);
    r->search.type = YEW_REG_CHARWISE;
...
{
    Bytebuf rebuilt;
    YewRegRowVec rows = {NULL, 0U, 0U};
    u32 result_width = (u32)yew_ccol_max((CCol){dst->width},
                                         (CCol){src->width}).v;
    ...
    for (p = 0U; p < YEW_ARRAY_LEN(parts); p++) {
        ...
        for (i = 0U; i < part->rows.len; i++) {
            ...
            if (row.lo > row.hi || row.hi > part->bytes.len)
                YEW_BUG("register: invalid block row span");
            next.lo = rebuilt.len;
            bytebuf_append(&rebuilt, part->bytes.data + row.lo,
                           (size_t)(row.hi - row.lo));
            if (!part->ragged) {
                u64 padding = yew_ccol_shortfall((CCol){result_width},
                                                  (CCol){part->width});
                while (padding != 0U) {
            ...
            next.hi = rebuilt.len;
            YewRegRowVec_push(&rows, next);
        }
    }
    bytebuf_free(&dst->bytes);
    YewRegRowVec_free(&dst->rows);
    dst->bytes = rebuilt;
    dst->rows = rows;
    dst->width = result_width;
    dst->ragged = result_ragged;
    dst->type = YEW_REG_BLOCKWISE;
    dst->t_wall = (i64)time(NULL);
}
...
void yew_reg_append(Registers *r, u8 name, const RegVal *v)
{
    ...
    if (r == NULL || v == NULL)
        YEW_BUG("yew_reg_append: NULL argument");
    if (name < 'A' || name > 'Z')
        YEW_BUG("register append requires A-Z");
    lower = (u8)(name - 'A' + 'a');
    dst = yew_reg_get(r, lower);
    if (dst->bytes.len == 0U && dst->rows.len == 0U) {
        reg_set_raw(dst, v);
        return;
    }
    if (dst->type == YEW_REG_BLOCKWISE || v->type == YEW_REG_BLOCKWISE) {
        if (dst->type == YEW_REG_BLOCKWISE &&
            v->type == YEW_REG_BLOCKWISE) {
            append_block(dst, v);
        } else {
    ...
    if (dst->type == YEW_REG_CHARWISE && v->type == YEW_REG_LINEWISE) {
        append_eol(&dst->bytes, v);
        bytebuf_append(&dst->bytes, v->bytes.data, v->bytes.len);
        dst->type = YEW_REG_LINEWISE;
    } else if (dst->type == YEW_REG_LINEWISE &&
               v->type == YEW_REG_CHARWISE) {
        u8 eol[2];
        size_t eol_len = eol_len_at_end(dst);
        if (eol_len == 0U)
            YEW_BUG("linewise register payload lacks trailing EOL");
...
static void ring_drop_oldest(Registers *r)
{
    ...
    if (r->ring_len == 0U)
        return;
    index = (r->ring_head + YEW_KILL_RING_MAX - (r->ring_len - 1U)) %
            YEW_KILL_RING_MAX;
...
{
    if (r == NULL)
        YEW_BUG("yew_reg_ring_set_depth: NULL register file");
    if (depth > YEW_KILL_RING_MAX)
        YEW_BUG("yew_reg_ring_set_depth: depth exceeds maximum");
    r->paste_live = false;
...
void yew_reg_ring_push(Registers *r, const RegVal *v)
{
    ...
    if (r == NULL || v == NULL)
        YEW_BUG("yew_reg_ring_push: NULL argument");
    r->paste_live = false;
    depth = r->ring_depth > YEW_KILL_RING_MAX ? YEW_KILL_RING_MAX :
                                               r->ring_depth;
    if (depth == 0U)
        return;
    while (r->ring_len >= depth)
    ...
    index = r->ring_len == 0U ? r->ring_head :
            (r->ring_head + 1U) % YEW_KILL_RING_MAX;
    r->ring_head = index;
...
void yew_reg_yank(Registers *r, u8 explicit_name, const RegVal *v)
{
    ...
    if (r == NULL || v == NULL)
        YEW_BUG("yew_reg_yank: NULL argument");
    if (explicit_name == '_')
        return;
    ...
    if (system || r->clipboard_sync == YEW_CLIP_SYNC_YANK ||
        r->clipboard_sync == YEW_CLIP_SYNC_ALL ||
        r->clipboard_sync == YEW_CLIP_SYNC_UNNAMED) {
        reg_set_raw(&r->system, v);
        (void)yew_clip_write(v, explicit_name == '*' ? '*' : '+');
    }
}
...
void yew_reg_delete(Registers *r, u8 explicit_name, const RegVal *v)
{
    ...
    if (r == NULL || v == NULL)
        YEW_BUG("yew_reg_delete: NULL argument");
    if (explicit_name == '_')
        return;
    system = explicit_name == '+' || explicit_name == '*';
    set_explicit(r, explicit_name, v);
    set_unnamed(r, v);
    if (v->type == YEW_REG_LINEWISE || contains_lf(v)) {
        for (i = 9U; i > 1U; i--)
    ...
    if (system || r->clipboard_sync == YEW_CLIP_SYNC_ALL ||
        r->clipboard_sync == YEW_CLIP_SYNC_UNNAMED) {
        reg_set_raw(&r->system, v);
        (void)yew_clip_write(v, explicit_name == '*' ? '*' : '+');
    }
...
static void remember_insert(Registers *r, ByteOff at, u64 len)
{
    ...
    span.lo = at.v;
    span.hi = at.v + len;
    YewRegPasteSpanVec_push(&r->paste_spans, span);
}
                       ...
                       bool before)
{
    ...
    LineNo line = yew_textbuf_line_of(ec->tb, cursor.pos);
    Span span = yew_textbuf_line_span(ec->tb, line);
    ...
    if (before)
        ...
        at = yew_grapheme_next(ec->tb, cursor.pos);
    if (!yew_edit_insert(ec, at, v->bytes.data, v->bytes.len))
        return false;
    remember_insert(r, at, v->bytes.len);
    end = BYTEOFF(at.v + v->bytes.len);
    set_primary_cursor(ec, yew_grapheme_prev(ec->tb, end));
    return true;
}
                       ...
                       bool before)
{
    ...
    if (!ends_in_lf(&v->bytes))
        YEW_BUG("linewise register payload lacks trailing EOL");
    if (before) {
        at = yew_textbuf_line_start(ec->tb, line);
        if (!yew_edit_insert(ec, at, v->bytes.data, v->bytes.len))
            return false;
        remember_insert(r, at, v->bytes.len);
        ...
        if (ec->meta == NULL)
            YEW_BUG("linewise paste requires file metadata");
        at = BYTEOFF(yew_textbuf_len(ec->tb));
        yew_filemeta_eol_bytes(ec->meta, &eol, &eol_len);
        if (!yew_edit_insert(ec, at, eol, eol_len))
            return false;
        remember_insert(r, at, eol_len);
        if (v->bytes.len > payload_eol) {
            ByteOff payload_at = BYTEOFF(at.v + eol_len);
            if (!yew_edit_insert(ec, payload_at, v->bytes.data,
...
static ByteOff block_target(const TextBuf *tb, Cursor cursor, bool before)
{
    ...
    return yew_grapheme_next(tb, cursor.pos);
}
...
static bool insert_spaces(Registers *r, EditCtx *ec, ByteOff at, u64 count)
{
    ...
    while (done < count) {
        u64 take = count - done < sizeof(spaces) ? count - done :
                                                     sizeof(spaces);
        if (!yew_edit_insert(ec, BYTEOFF(at.v + done), spaces, take))
            return false;
...
{
    Cursor cursor = ec->cset->curs.data[ec->cset->primary];
    LineNo first_line = yew_textbuf_line_of(ec->tb, cursor.pos);
    ByteOff target = block_target(ec->tb, cursor, before);
    Span first_span = yew_textbuf_line_span(ec->tb, first_line);
    CCol column = yew_off_to_ccol(ec->tb, first_span, target, tabw);
    ...
    for (i = 0U; i < v->rows.len; i++) {
        ...
        while (line.v >= yew_textbuf_line_count(ec->tb)) {
            const u8 *eol;
            size_t eol_len;
            ByteOff end = BYTEOFF(yew_textbuf_len(ec->tb));
            if (ec->meta == NULL)
                YEW_BUG("blockwise line extension requires file metadata");
            yew_filemeta_eol_bytes(ec->meta, &eol, &eol_len);
            if (!yew_edit_insert(ec, end, eol, eol_len))
                return false;
            remember_insert(r, end, eol_len);
        }
        if (row.lo > row.hi || row.hi > v->bytes.len)
            YEW_BUG("register: invalid block row span");
        dst = yew_textbuf_line_span(ec->tb, line);
        off = yew_ccol_to_off_padded(ec->tb, dst, column, tabw);
        landed = yew_off_to_ccol(ec->tb, dst, off, tabw);
        pad = yew_ccol_shortfall(column, landed);
        if (!insert_spaces(r, ec, off, pad))
            return false;
        off.v += pad;
        if (!yew_edit_insert(ec, off, v->bytes.data + row.lo,
                             row.hi - row.lo))
            return false;
...
static bool paste_value(Registers *r, EditCtx *ec, const RegVal *v,
                        bool before, u32 tabw)
{
    if (v->type == YEW_REG_CHARWISE)
        return paste_char(r, ec, v, before);
    if (v->type == YEW_REG_LINEWISE)
        return paste_line(r, ec, v, before);
    if (v->type == YEW_REG_BLOCKWISE)
        return paste_block(r, ec, v, before, tabw);
    YEW_BUG("register: invalid value type");
}
...
static u32 find_ring_value(const Registers *r, const RegVal *v)
{
    ...
    for (i = 0U; i < r->ring_len; i++) {
        u32 index = (r->ring_head + YEW_KILL_RING_MAX - i) %
                    YEW_KILL_RING_MAX;
        if (&r->ring[index] == v)
            return index;
    }
                   ...
                   u32 tabw)
{
    ...
    if (r == NULL || ec == NULL || ec->tb == NULL || ec->undo == NULL ||
        ec->cset == NULL)
        YEW_BUG("yew_reg_paste: incomplete context");
    if (name == '+' || name == '*') {
        yew_clip_set_read_max(r->clip_read_max);
        if (!yew_clip_read(&r->system, name == '*' ? '*' : '+'))
            return false;
        name = '+';
    } else if ((name == 0U || name == '"') &&
               r->clipboard_sync == YEW_CLIP_SYNC_UNNAMED) {
        yew_clip_set_read_max(r->clip_read_max);
        if (!yew_clip_read(&r->system, '+'))
            return false;
        set_unnamed(r, &r->system);
        name = '"';
    }
    v = yew_reg_get(r, name);
    if (v == NULL)
        return false;
...
bool yew_reg_ring_cycle(Registers *r, EditCtx *ec, i32 delta)
{
    ...
    if (r == NULL || ec == NULL || !r->paste_live || r->ring_len == 0U ||
        ...
        r->paste_win_id != ec->win_id)
        return false;
    if (!yew_undo_reopen(ec, YEW_TXN_PASTE)) {
        r->paste_live = false;
        yew_log(YEW_LOG_WARN, "register: ring cycle refused after edit");
        return false;
    }
    i = r->paste_spans.len;
    while (i != 0U)
        yew_edit_delete(ec, r->paste_spans.data[--i]);
    set_primary_cursor(ec, r->paste_origin.pos);
    current_rank = (r->ring_head + YEW_KILL_RING_MAX -
                    r->paste_ring_index) % YEW_KILL_RING_MAX;
    ...
    rank %= r->ring_len;
    r->paste_ring_index =
        (r->ring_head + YEW_KILL_RING_MAX - (u32)rank) %
        YEW_KILL_RING_MAX;
    r->paste_spans.len = 0U;
