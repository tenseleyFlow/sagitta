#ifndef SAG_WS_WORKSPACE_H
#define SAG_WS_WORKSPACE_H
 ...
 * looks at ~/.local/state sees no trace of us.  The one file sagitta
 * ever reads from a workspace is `.sagitta.fl`, which is user-authored
...
typedef struct WsKey {
    ...
    char dir[PATH_MAX];      /* $XDG_STATE_HOME/sagitta/workspaces/… */
...
const char *sag_ws_state_path(const WsKey *k); /* <dir>/state.fl */
const char *sag_ws_undo_dir(const WsKey *k);   /* <dir>/undo/    */
const char *sag_ws_lock_path(const WsKey *k);  /* <dir>/lock     */
