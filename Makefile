...
UNIT_FUSS_SRC := tests/unit/test_porcelain.c tests/unit/test_gitcache.c \
                 ...
                 tests/unit/test_fussexpand.c tests/unit/test_fussjump.c \
...
FUZZ_WATCHDOG_OBJ := $(BUILD)/tests/fuzz/fuzz_watchdog.o
FUZZ_WATCHDOG := $(BUILD)/fuzz_watchdog
...
BUILD_DIRS := $(sort $(dir $(OBJ) $(UNIT_OBJ) $(AUDIT_OBJ) \
                ...
                $(FUZZ_COV_CALIB_OBJ) $(FUZZ_WATCHDOG_OBJ) \
...
$(FUZZ_WATCHDOG): $(FUZZ_LINK_OBJ) $(FUZZ_WATCHDOG_OBJ)
	$(CC) $(CFLAGS) $(LDFLAGS) -o $@ $(FUZZ_LINK_OBJ) \
		$(FUZZ_WATCHDOG_OBJ) $(LDLIBS)
...
fuzzlib-selftest: $(BUILD)/fuzz_utf8 $(FUZZ_WATCHDOG)
	@set -eu; \
	mkdir -p $(BUILD)/tmp; \
	...
	scripts/fuzz-watchdog-selftest.sh $(FUZZ_WATCHDOG); \
...
# Sprint 57.23: the shell context lexer.  Seeded from the unit corpus
# (tests/fuzz/corpus/fuzz_shctx/); SHCTX_FUZZ_SECONDS=60 is the DoD run.
