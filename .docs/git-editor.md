...
In F mode, press `D` on a file, or run `ed.git.diff.view`, to open a vertical
side-by-side diff. The indexed version is on the left and the live buffer is
on the right. Both panes are read-only scratch buffers.
...
In F mode, press `g` on a directory, or run `ed.group.from_dir` with a path.
Yew walks recursively by default, skips `.git`, respects ignore rules, and
orders members deterministically. An already-open tab is adopted rather than
duplicated. New tabs remain deferred; yew reads only the first focused file.
