...
| Placement | Clamp, never flip (unchanged). Pointer-opened menus place the box's top-left border cell **at the click cell**. Keyboard-opened menus retain the below-right focus offset. In both cases the opening release lands on a border, never an action row. |
