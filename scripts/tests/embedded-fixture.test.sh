...
# gen-bigfile reports its output path on success.  The randomized temporary
# path is useful interactively but must not leak into deterministic test logs;
# the checks below independently prove the generated bytes and hash-stable
# rerun.
"$repo/scripts/gen-embedded-fixture.sh" "$scratch/gen-bigfile" \
    "$scratch/one.c" >/dev/null
sleep 1
"$repo/scripts/gen-embedded-fixture.sh" "$scratch/gen-bigfile" \
    "$scratch/two.c" >/dev/null
