...
#   3  every native from `sag fl --list-natives` appears in a COVERS
...
if [ ! -x "$SAGITTA" ]; then
    echo "check-fletch-coverage: $SAGITTA is not built (make)" >&2
    exit 2
fi
...
# Checks 1-6.
if ! LC_ALL=C "$RUNNER" --check --sagitta "$SAGITTA"; then
    rc=1
fi
...
LC_ALL=C "$RUNNER" --ledger --sagitta "$SAGITTA" >"$tmp"
