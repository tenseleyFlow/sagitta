/* Generate yew's Unicode 16.0.0 property trie from checked-in UCD data. */
#include <ctype.h>
#include <errno.h>
#include <stdint.h>
#include <stdio.h>
    ...
    while (*p != '\0') {
        ...
        *end = '\0';
        if (count == CASE_MAX_CPS)
            die("case mapping exceeds YEW_CASE_MAX_CODEPOINTS", path);
...
static void emit_output(void)
{
    ...
    size_t i;
    /* The emitted YewURange ABI is three naturally aligned 32/32/16-bit
    ...
    fputs("/* GENERATED by scripts/gen-unicode-tables from UCD 16.0.0 — do not edit;\n"
          ...
          " */\n#include \"tables.h\"\n\n", stdout);
    emit_u16("yew_u_stage1[YEW_TRIE_HI >> YEW_TRIE_SHIFT]", stage1,
             STAGE1_LEN);
    emit_u8("yew_u_stage2[]", stage2, block_count * BLOCK_SIZE);
    emit_u16("yew_u_pal[]", palette, palette_count);
    fputs("const struct YewURange yew_u_hi[] = {\n", stdout);
    ...
    fputs("};\n\n", stdout);
    printf("const u32 yew_u_hi_len = %uu;\n\n", (unsigned)hi_count);
    fputs("u16 yew_u_hi_lookup(u32 cp)\n{\n"
          "    size_t lo = 0;\n"
          ...
          "    return 0;\n"
          "}\n\n", stdout);
    fputs("_Static_assert(sizeof(yew_u_stage1) + sizeof(yew_u_stage2) +\n"
...
static void emit_wb_output(void)
{
    ...
    fputs("/* GENERATED by scripts/gen-unicode-tables from UCD 16.0.0 — do not edit;\n"
          " * regenerate with 'make unicode-tables'.\n"
          " */\n#include \"wordbreak.h\"\n\n", stdout);
    emit_u16("yew_wb_stage1[YEW_WB_TRIE_HI >> YEW_WB_TRIE_SHIFT]",
             wb_stage1, STAGE1_LEN);
    emit_u8("yew_wb_stage2[]", wb_stage2,
            wb_block_count * BLOCK_SIZE);
    fputs("const struct YewWbRange yew_wb_hi[] = {\n", stdout);
    for (i = 0; i < wb_hi_count; i++)
        printf("    {0x%06Xu, 0x%06Xu, 0x%02Xu}%s\n",
               ...
               i + 1u == wb_hi_count ? "" : ",");
    fputs("};\n\n", stdout);
    printf("const u32 yew_wb_hi_len = %uu;\n\n", (unsigned)wb_hi_count);
    fputs("_Static_assert(sizeof(yew_wb_stage1) + sizeof(yew_wb_stage2) +\n"
...
static void emit_case_output(void)
{
    ...
    fputs("/* GENERATED by scripts/gen-unicode-tables from UCD 16.0.0 — do not edit;\n"
          ...
          "    u8 upper_len;\n"
          "};\n\n", stdout);
    fputs("static const u32 yew_case_data[] = {\n", stdout);
...
static void emit_cat_output(void)
{
    ...
    fputs("/* GENERATED by scripts/gen-unicode-tables from UCD 16.0.0 — do not edit;\n"
          " * regenerate with 'make unicode-tables'.\n"
          " */\n#include \"category.h\"\n\n", stdout);
    emit_u16("yew_cat_stage1[YEW_CAT_TRIE_HI >> YEW_CAT_TRIE_SHIFT]",
             cat_stage1, STAGE1_LEN);
    emit_u8("yew_cat_stage2[]", cat_stage2,
            cat_block_count * BLOCK_SIZE);
    emit_u16("yew_cat_pal[]", cat_palette, cat_palette_count);
    fputs("const struct YewCatRange yew_cat_hi[] = {\n", stdout);
    for (i = 0; i < cat_hi_count; i++)
        printf("    {0x%06Xu, 0x%06Xu, 0x%04Xu}%s\n",
               ...
               i + 1u == cat_hi_count ? "" : ",");
    fputs("};\n\n", stdout);
    printf("const u32 yew_cat_hi_len = %uu;\n\n", (unsigned)cat_hi_count);
    fputs("_Static_assert(sizeof(yew_cat_stage1) + sizeof(yew_cat_stage2) +\n"
