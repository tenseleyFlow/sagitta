...
static SagRe *ov_compile(Arena *a, const char *pat)
{
    SearchOpts o;
    SagRe *re;
    ...
    re = sag_search_compile(a, pat, strlen(pat), &o, NULL);
...
void test_overlay_scan_is_bounded_by_viewport_and_lookahead(void)
{
    ...
    SAG_ASSERT_EQ_U64(ed.win->overlay.spans.len, 1U);
    /* Line 2 starts at byte 10 (two "aaaa\n" before it). */
    SAG_ASSERT_EQ_U64(ed.win->overlay.spans.data[0].lo, 10U);
    /* And the scanned range really is bounded, not merely lucky. */
    SAG_ASSERT(ed.win->overlay.scanned.hi < sag_textbuf_len(ed.buffer.tb));
...
void test_overlay_lookahead_reaches_past_the_visible_lines(void)
{
    ...
    visible_hi = sag_textbuf_line_start(ed.buffer.tb, LINENO(11U)).v;
    SAG_ASSERT_EQ_U64(ed.win->overlay.spans.len, 1U);
    SAG_ASSERT(ed.win->overlay.spans.data[0].lo > visible_hi);
    SAG_ASSERT(ed.win->overlay.scanned.hi > visible_hi);
    arena_free_all(&arena);
    sag_ed_free(&ed);
}
...
void test_overlay_scroll_inside_scanned_range_is_reused(void)
{
    ...
    sag_overlay_refresh(&ed, ed.win, re, 1U, 0);
    SAG_ASSERT_EQ_U64(ed.win->overlay.scanned.lo, first_scanned.lo);
    SAG_ASSERT_EQ_U64(ed.win->overlay.scanned.hi, first_scanned.hi);
    SAG_ASSERT_EQ_U64(ed.win->overlay.spans.len, first_len);
    arena_free_all(&arena);
    sag_ed_free(&ed);
}
...
void test_overlay_invalidates_on_pattern_and_buffer_generation(void)
{
    ...
    SAG_ASSERT_EQ_U64(ed.win->overlay.pat_gen, 1U);

    /* A recompiled pattern: same generation number would be a stale
     * highlight, so the key must move. */
    sag_overlay_refresh(&ed, ed.win, re, 2U, 0);
    SAG_ASSERT_EQ_U64(ed.win->overlay.pat_gen, 2U);

    /* An edit bumps TextBuf.gen, which invalidates independently. */
    ec = sag_ed_edit_ctx(&ed);
    SAG_ASSERT(sag_edit_insert(&ec, BYTEOFF(0U), (const u8 *)"needle\n",
                               7U));
    sag_overlay_refresh(&ed, ed.win, re, 2U, 0);
    SAG_ASSERT_EQ_U64(ed.win->overlay.buf_gen, ed.buffer.tb->gen);
    SAG_ASSERT_EQ_U64(ed.win->overlay.spans.len, 3U);
    arena_free_all(&arena);
    sag_ed_free(&ed);
}
...
void test_overlay_spans_are_ascending_and_disjoint(void)
{
    ...
    re = ov_compile(&arena, "a*");
    sag_overlay_refresh(&ed, ed.win, re, 1U, 0);
    len = sag_textbuf_len(ed.buffer.tb);
    SAG_ASSERT(ed.win->overlay.spans.len > 0U);
    for (i = 0U; i < ed.win->overlay.spans.len; i++) {
        Span s = ed.win->overlay.spans.data[i];

        SAG_ASSERT(s.lo <= s.hi);
        SAG_ASSERT(s.hi <= len);
        if (i > 0U)
            SAG_ASSERT(ed.win->overlay.spans.data[i - 1U].hi <= s.lo);
    }
    arena_free_all(&arena);
    sag_ed_free(&ed);
}
...
void test_overlay_null_pattern_clears(void)
{
    ...
    sag_overlay_refresh(&ed, ed.win, re, 1U, 0);
    SAG_ASSERT(ed.win->overlay.spans.len > 0U);
    sag_overlay_refresh(&ed, ed.win, NULL, 1U, 0);
...
void test_overlay_count_caps_at_ten_thousand(void)
{
    ...
    sag_overlay_count(&ed.win->overlay, re, ed.buffer.tb, 0);
    SAG_ASSERT_EQ_U64(ed.win->overlay.count_total, SAG_SEARCH_COUNT_MAX);
    SAG_ASSERT(ed.win->overlay.count_capped);
    arena_free_all(&arena);
    sag_ed_free(&ed);
}
...
void test_overlay_count_is_exact_below_the_cap(void)
{
    ...
    re = ov_compile(&arena, "needle");
    sag_overlay_count(&ed.win->overlay, re, ed.buffer.tb, 0);
    SAG_ASSERT_EQ_U64(ed.win->overlay.count_total, 2U);
    SAG_ASSERT(!ed.win->overlay.count_capped);
    arena_free_all(&arena);
    sag_ed_free(&ed);
}
...
void test_overlay_damage_is_diffed_not_blanket(void)
{
    ...
    sag_overlay_refresh(&ed, ed.win, wide, 1U, 0);
    SAG_ASSERT_EQ_U64(ed.win->overlay.spans.len, 2U);

    ed.doc_damage_lo = 0U;
    ed.doc_damage_hi = 0U;
    narrow = sag_re_compile(&arena, "needle", 6U, 0U, NULL);
    SAG_ASSERT_NOT_NULL(narrow);
    sag_overlay_refresh(&ed, ed.win, narrow, 2U, 0);
    SAG_ASSERT_EQ_U64(ed.win->overlay.spans.len, 2U);
    ...
    for (i = 0U; i < ed.win->overlay.spans.len; i++)
        changed_lines++;
    SAG_ASSERT_EQ_U64(changed_lines, 2U);
    SAG_ASSERT(ed.doc_damage_hi - ed.doc_damage_lo <= ed.win->rect.h);
    arena_free_all(&arena);
    sag_ed_free(&ed);
}
