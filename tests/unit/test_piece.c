...
static Bytebuf textbuf_materialize(const TextBuf *tb)
{
    ...
    if (yew_textiter_begin(&it, tb, BYTEOFF(0U))) {
        do {
            ...
            YEW_ASSERT(yew_textiter_chunk(&it, tb, &chunk, &len));
            YEW_ASSERT(len > 0U);
                                ...
                                const void *expected, size_t len)
{
    ...
    YEW_ASSERT_EQ_U64(snap->len, len);
    YEW_ASSERT_EQ_U64(actual.len, len);
    YEW_ASSERT_EQ_MEM(actual.data, expected, len);
    bytebuf_free(&actual);
}
...
void test_piece_construct_empty(void)
{
    ...
    YEW_ASSERT_EQ_U64(yew_textbuf_line_count(tb), 1U);
    YEW_ASSERT_EQ_U64(yew_textbuf_piece_count(tb), 0U);
...
void test_piece_construct_bytes(void)
{
    ...
    TextBuf *tb = yew_textbuf_from_bytes(source, sizeof(source) - 1U);

    YEW_ASSERT_NOT_NULL(tb);
    source[0] = (u8)'X';
    yew_textbuf_check(tb);
    YEW_ASSERT_EQ_U64(yew_textbuf_line_count(tb), 2U);
    YEW_ASSERT_EQ_U64(yew_textbuf_piece_count(tb), 1U);
...
{
    u8 *bytes = yew_xmalloc(4U);
    TextBuf *tb;

    (void)memcpy(bytes, "a\nb", 4U);
    tb = yew_textbuf_from_owned_bytes(bytes, 4U);
    YEW_ASSERT(tb->orig.bytes == bytes);
    YEW_ASSERT_EQ_U64(yew_textbuf_len(tb), 4U);
    YEW_ASSERT_EQ_U64(yew_textbuf_line_count(tb), 2U);
...
{
    TextBuf *front = yew_textbuf_from_bytes((const u8 *)"bc", 2U);
    TextBuf *middle = yew_textbuf_from_bytes((const u8 *)"ac", 2U);
    TextBuf *end = yew_textbuf_from_bytes((const u8 *)"ab", 2U);

    yew_textbuf_insert(front, BYTEOFF(0U), (const u8 *)"a", 1U);
    yew_textbuf_check(front);
    assert_content(front, "abc", 3U);
    yew_textbuf_insert(middle, BYTEOFF(1U), (const u8 *)"b", 1U);
    yew_textbuf_check(middle);
    assert_content(middle, "abc", 3U);
    yew_textbuf_insert(end, BYTEOFF(2U), (const u8 *)"c", 1U);
    yew_textbuf_check(end);
    assert_content(end, "abc", 3U);
    yew_textbuf_free(front);
    yew_textbuf_free(middle);
    yew_textbuf_free(end);
}

static void assert_insert_span_reuses_store(u8 src)
{
    TextBuf *tb = yew_textbuf_from_bytes((const u8 *)"orig\n", 5U);
    Span span;
    u64 add_len;
    u64 gen;

    yew_textbuf_insert(tb, BYTEOFF(5U), (const u8 *)"add\n", 4U);
    span = src == YEW_STORE_ORIG ? (Span){0U, 5U} : (Span){0U, 4U};
    add_len = tb->add.len;
    gen = tb->gen;
    yew_textbuf_insert_span(tb, BYTEOFF(0U), src, span);
    yew_textbuf_check(tb);
    YEW_ASSERT_EQ_U64(tb->add.len, add_len);
    YEW_ASSERT_EQ_U64(tb->gen, gen + 1U);
    YEW_ASSERT_EQ_U64(yew_textbuf_line_count(tb), 4U);
...
{
    TextBuf *left_seam = yew_textbuf_from_bytes((const u8 *)"abcdef", 6U);
    TextBuf *right_seam = yew_textbuf_from_bytes((const u8 *)"abcdef", 6U);

    yew_textbuf_insert(left_seam, BYTEOFF(3U), (const u8 *)"X", 1U);
    yew_textbuf_check(left_seam);
    YEW_ASSERT_EQ_U64(yew_textbuf_piece_count(left_seam), 3U);
    yew_textbuf_insert(left_seam, BYTEOFF(3U), (const u8 *)"L", 1U);
    yew_textbuf_check(left_seam);
    assert_content(left_seam, "abcLXdef", 8U);

    yew_textbuf_insert(right_seam, BYTEOFF(3U), (const u8 *)"X", 1U);
    yew_textbuf_check(right_seam);
    YEW_ASSERT_EQ_U64(yew_textbuf_piece_count(right_seam), 3U);
    yew_textbuf_insert(right_seam, BYTEOFF(4U), (const u8 *)"R", 1U);
    yew_textbuf_check(right_seam);
    assert_content(right_seam, "abcXRdef", 8U);
    yew_textbuf_free(left_seam);
    yew_textbuf_free(right_seam);

    assert_insert_span_reuses_store(YEW_STORE_ORIG);
    assert_insert_span_reuses_store(YEW_STORE_ADD);
}
...
void test_piece_insert_span_recoalesces_boundaries(void)
{
    ...
    yew_textbuf_delete(tb, (Span){2U, 4U});
    yew_textbuf_check(tb);
    YEW_ASSERT_EQ_U64(yew_textbuf_piece_count(tb), 2U);
    yew_textbuf_insert_span(tb, BYTEOFF(2U), YEW_STORE_ORIG,
                            (Span){2U, 4U});
    yew_textbuf_check(tb);
    YEW_ASSERT_EQ_U64(yew_textbuf_piece_count(tb), 1U);
...
void test_piece_insert_accepts_exposed_chunk_alias(void)
{
    ...
    yew_textbuf_insert(tb, BYTEOFF(0U), (const u8 *)"a", 1U);
    yew_textbuf_check(tb);
    fill_len = tb->add.cap - tb->add.len;
    fill = yew_xmalloc((size_t)fill_len);
    ...
    YEW_ASSERT(yew_textiter_chunk(&it, tb, &aliased, &aliased_len));
    YEW_ASSERT_EQ_U64(aliased_len, before.len);
    yew_textbuf_insert(tb, BYTEOFF(yew_textbuf_len(tb)), aliased, aliased_len);
    yew_textbuf_check(tb);
    after = textbuf_materialize(tb);
    YEW_ASSERT_EQ_U64(after.len, before.len * 2U);
    YEW_ASSERT_EQ_MEM(after.data, before.data, before.len);
    YEW_ASSERT_EQ_MEM(after.data + before.len, before.data, before.len);
    bytebuf_free(&before);
    bytebuf_free(&after);
    yew_textbuf_free(tb);
}
...
void test_piece_insert_recoalesces_after_delete(void)
{
    ...
    yew_textbuf_insert(tb, BYTEOFF(0U), (const u8 *)"ab", 2U);
    yew_textbuf_check(tb);
    yew_textbuf_delete(tb, (Span){0U, 1U});
    yew_textbuf_check(tb);
    yew_textbuf_insert(tb, BYTEOFF(1U), (const u8 *)"c", 1U);
    yew_textbuf_check(tb);
    YEW_ASSERT_EQ_U64(yew_textbuf_piece_count(tb), 1U);
...
void test_piece_delete_single_byte(void)
{
    ...
    yew_textbuf_delete(tb, (Span){1U, 2U});
    yew_textbuf_check(tb);
...
void test_piece_delete_cross_piece(void)
{
    ...
    yew_textbuf_insert(tb, BYTEOFF(3U), (const u8 *)"XYZ", 3U);
    yew_textbuf_check(tb);
    yew_textbuf_delete(tb, (Span){2U, 7U});
    yew_textbuf_check(tb);
...
void test_piece_delete_whole_buffer(void)
{
    ...
    yew_textbuf_delete(tb, (Span){0U, 7U});
    yew_textbuf_check(tb);
    YEW_ASSERT_EQ_U64(yew_textbuf_line_count(tb), 1U);
    YEW_ASSERT_EQ_U64(yew_textbuf_piece_count(tb), 0U);
    assert_content(tb, NULL, 0U);
...
void test_piece_delete_empty_range(void)
{
    ...
    yew_textbuf_delete(tb, (Span){1U, 1U});
    yew_textbuf_check(tb);
    YEW_ASSERT_EQ_U64(tb->gen, gen);
...
void test_piece_delete_recoalesces_adjacent_spans(void)
{
    ...
    yew_textbuf_check(tb);
    YEW_ASSERT_EQ_U64(yew_textbuf_piece_count(tb), 3U);
    yew_textbuf_delete(tb, (Span){3U, 4U});
    yew_textbuf_check(tb);
    YEW_ASSERT_EQ_U64(yew_textbuf_piece_count(tb), 1U);
    assert_content(tb, "abcdef", 6U);
    yew_textbuf_free(tb);
...
void test_piece_iterator_chunks_are_maximal_after_delete(void)
{
    ...
    YEW_ASSERT(yew_textiter_begin(&it, tb, BYTEOFF(0U)));
    YEW_ASSERT(yew_textiter_chunk(&it, tb, &chunk, &len));
    YEW_ASSERT_EQ_U64(len, 6U);
    YEW_ASSERT_EQ_MEM(chunk, "abcdef", 6U);
...
void test_piece_line_mapping_edges(void)
{
    ...
    TextBuf *tb = yew_textbuf_from_bytes(with_eof_lf,
                                         sizeof(with_eof_lf) - 1U);
    TextBuf *no_eof = yew_textbuf_from_bytes((const u8 *)"a\nlast", 6U);
    size_t i;

    yew_textbuf_check(tb);
    YEW_ASSERT_EQ_U64(yew_textbuf_line_count(tb), YEW_ARRAY_LEN(starts));
    for (i = 0U; i < YEW_ARRAY_LEN(starts); i++) {
        ByteOff start = yew_textbuf_line_start(tb, LINENO(i));
        Span span = yew_textbuf_line_span(tb, LINENO(i));

        YEW_ASSERT_EQ_U64(start.v, starts[i]);
        YEW_ASSERT_EQ_U64(yew_textbuf_line_of(tb, start).v, i);
        YEW_ASSERT_EQ_U64(span.lo, spans[i].lo);
        YEW_ASSERT_EQ_U64(span.hi, spans[i].hi);
    }
    YEW_ASSERT_EQ_U64(yew_textbuf_line_span(no_eof, LINENO(1U)).lo, 2U);
    YEW_ASSERT_EQ_U64(yew_textbuf_line_span(no_eof, LINENO(1U)).hi, 6U);
    YEW_ASSERT_EQ_U64(yew_textbuf_line_of(no_eof, BYTEOFF(6U)).v, 1U);
    yew_textbuf_free(tb);
    yew_textbuf_free(no_eof);
}

void test_piece_line_mapping_mixed_fixture(void)
{
    enum { FIXTURE_LEN = 64 * 1024 };
    u8 *fixture = yew_xmalloc(FIXTURE_LEN);
    TextBuf *tb;
    u64 off;
    ...
    for (off = 0U; off <= FIXTURE_LEN; off++) {
        ...
        YEW_ASSERT(start.v <= off);
        YEW_ASSERT_EQ_U64(yew_textbuf_line_of(tb, start).v, line.v);
    }
    yew_textbuf_free(tb);
}
...
void test_piece_line_iterator_yields_spans(void)
{
    ...
    YEW_ASSERT(yew_lineiter_begin(&it, tb, LINENO(1U)));
    while (yew_lineiter_next(&it, tb, &span)) {
        YEW_ASSERT(i < YEW_ARRAY_LEN(expected));
        YEW_ASSERT_EQ_U64(span.lo, expected[i].lo);
        YEW_ASSERT_EQ_U64(span.hi, expected[i].hi);
        i++;
    }
    YEW_ASSERT_EQ_U64(i, YEW_ARRAY_LEN(expected));
    yew_textbuf_free(tb);
}
...
void test_piece_sequential_insert_coalesces(void)
{
    ...
    for (i = 0U; i < 10000U; i++) {
        yew_textbuf_insert(tb, BYTEOFF(i), (const u8 *)"q", 1U);
        yew_textbuf_check(tb);
    }
    YEW_ASSERT(yew_textbuf_piece_count(tb) <= 3U);
    actual = textbuf_materialize(tb);
    YEW_ASSERT_EQ_U64(actual.len, 10000U);
...
void test_piece_interleaved_insert_stays_distinct(void)
{
    ...
    for (i = 0U; i < 100U; i++) {
        ...
        yew_textbuf_insert(tb, BYTEOFF(0U), &byte, 1U);
        yew_textbuf_check(tb);
    }
    YEW_ASSERT(yew_textbuf_piece_count(tb) > 3U);
    YEW_ASSERT_EQ_U64(yew_textbuf_len(tb), 100U);
    yew_textbuf_free(tb);
}
...
void test_piece_snapshot_preserves_content(void)
{
    ...
    TextBuf *tb = yew_textbuf_from_bytes(original, sizeof(original) - 1U);
    TextSnap snap = yew_textbuf_snap(tb);
    u64 i;

    for (i = 0U; i < 100U; i++) {
        u8 byte = (u8)('A' + i % 26U);

        yew_textbuf_insert(tb, BYTEOFF(yew_textbuf_len(tb)), &byte, 1U);
        yew_textbuf_check(tb);
    }
...
void test_piece_snapshot_survives_middle_edits(void)
{
    ...
    u8 *initial = yew_xmalloc(INITIAL_LEN);
    TextBuf *tb;
    TextSnap snap;
    Bytebuf expected;
...
{
    TextBuf *tb = yew_textbuf_from_bytes((const u8 *)"pinned", 6U);
    TextSnap snap = yew_textbuf_snap(tb);
...
{
    TextBuf *tb = yew_textbuf_from_bytes((const u8 *)"old root", 8U);
    TextSnap snap = yew_textbuf_snap(tb);
...
{
    yew_textbuf_check(context);
}

void test_piece_checker_rejects_corruption(void)
{
    TextBuf *tb = yew_textbuf_from_bytes((const u8 *)"broken", 6U);
    u64 valid_sub_bytes = tb->root->sub_bytes;

    tb->root->sub_bytes++;
    YEW_ASSERT_EQ_I64(bug_action_exit(check_buffer_action, tb), YEW_EXIT_BUG);
...
static void stale_iterator_action(void *context)
{
    ...
    (void)yew_textiter_chunk(stale->it, stale->tb, &chunk, &len);
}
...
void test_piece_live_iterator_rejects_edit(void)
{
    ...
    YEW_ASSERT(yew_textiter_begin(&it, tb, BYTEOFF(0U)));
    yew_textbuf_insert(tb, BYTEOFF(1U), (const u8 *)"X", 1U);
    yew_textbuf_check(tb);
    context.tb = tb;
    context.it = &it;
    YEW_ASSERT_EQ_I64(bug_action_exit(stale_iterator_action, &context),
                      YEW_EXIT_BUG);
    yew_textbuf_free(tb);
}
...
static void wrong_line_iterator_action(void *context)
{
    ...
    (void)yew_lineiter_next(wrong->it, wrong->tb, &line);
}

void test_piece_line_iterator_rejects_other_buffer(void)
{
    TextBuf *first = yew_textbuf_from_bytes((const u8 *)"a\n", 2U);
    TextBuf *second = yew_textbuf_from_bytes((const u8 *)"b\n", 2U);
    LineIter it;
    WrongLineIterContext context;

    YEW_ASSERT(yew_lineiter_begin(&it, first, LINENO(0U)));
    context.tb = second;
    context.it = &it;
    YEW_ASSERT_EQ_I64(bug_action_exit(wrong_line_iterator_action, &context),
                      YEW_EXIT_BUG);
    yew_textbuf_check(first);
    yew_textbuf_check(second);
    yew_textbuf_free(first);
    yew_textbuf_free(second);
}
