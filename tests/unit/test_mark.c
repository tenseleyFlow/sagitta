...
void test_mark_generational_handles(void)
{
    ...
    hi_left = sag_mark_add(ms, BYTEOFF(8U), SAG_BIAS_LEFT);
    hi_right = sag_mark_add(ms, BYTEOFF(8U), SAG_BIAS_RIGHT);
    sag_marks_observe_collapse(ms, (Span){4U, 8U}, record_collapse, &log);
    SAG_ASSERT_EQ_U64(log.len, 4U);
    ...
    SAG_ASSERT_EQ_U64(log.ids[3].id, hi_left.id);
    SAG_ASSERT_EQ_U64(log.rel[3], 4U);
    ...
    SAG_ASSERT_EQ_U64(sag_mark_pos(ms, hi_left).v, 4U);
    SAG_ASSERT_EQ_U64(sag_mark_pos(ms, hi_right).v, 4U);
    ...
    SAG_ASSERT_EQ_U64(sag_mark_pos(ms, hi_left).v, 4U);
    SAG_ASSERT_EQ_U64(sag_mark_pos(ms, hi_right).v, 8U);
    ...
    SAG_ASSERT(sag_mark_repair(ms, hi_left, BYTEOFF(8U)));
    SAG_ASSERT_EQ_U64(sag_mark_pos(ms, edge).v, 4U);
    SAG_ASSERT_EQ_U64(sag_mark_pos(ms, inside_a).v, 5U);
    SAG_ASSERT_EQ_U64(sag_mark_pos(ms, inside_b).v, 7U);
    SAG_ASSERT_EQ_U64(sag_mark_pos(ms, hi_left).v, 8U);
    SAG_ASSERT_EQ_U64(sag_mark_pos(ms, hi_right).v, 8U);
