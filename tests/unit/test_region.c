...
void test_region_hit_finds_the_registered_rect(void)
{
    ...
    sag_region_add(SAG_REGION_PANE, (Rect){10U, 5U, 20U, 8U}, 3);
    SAG_ASSERT_EQ_U64(sag_region_count(), 1U);

    got = sag_region_hit(10U, 5U);
    SAG_ASSERT_EQ_U64(got.kind, SAG_REGION_PANE);
    SAG_ASSERT_EQ_I64(got.payload, 3);
    /* The far corner is inside; one past it is not — a half-open rect,
     * the same convention Span uses. */
    got = sag_region_hit(29U, 12U);
    SAG_ASSERT_EQ_U64(got.kind, SAG_REGION_PANE);
    got = sag_region_hit(30U, 12U);
    SAG_ASSERT_EQ_U64(got.kind, SAG_REGION_NONE);
    got = sag_region_hit(29U, 13U);
    SAG_ASSERT_EQ_U64(got.kind, SAG_REGION_NONE);
    got = sag_region_hit(9U, 5U);
    SAG_ASSERT_EQ_U64(got.kind, SAG_REGION_NONE);
}
...
void test_region_last_added_wins_on_overlap(void)
{
    ...
    sag_region_frame_begin();
    sag_region_add(SAG_REGION_PANE, (Rect){0U, 0U, 80U, 24U}, 1);
    sag_region_add(SAG_REGION_BLOCK, (Rect){10U, 10U, 20U, 4U}, 0);

    got = sag_region_hit(15U, 11U);
    SAG_ASSERT_EQ_U64(got.kind, SAG_REGION_BLOCK);
    /* Outside the overlay the pane still answers. */
    got = sag_region_hit(5U, 5U);
    SAG_ASSERT_EQ_U64(got.kind, SAG_REGION_PANE);
    SAG_ASSERT_EQ_I64(got.payload, 1);
}
...
void test_region_frame_begin_clears_the_table(void)
{
    ...
    sag_region_frame_begin();
    sag_region_add(SAG_REGION_PANE, (Rect){0U, 0U, 80U, 24U}, 7);
    SAG_ASSERT_EQ_U64(sag_region_hit(1U, 1U).kind, SAG_REGION_PANE);

    /* A frame that draws nothing — an early return, a resize bail. */
    sag_region_frame_begin();
    SAG_ASSERT_EQ_U64(sag_region_count(), 0U);
    got = sag_region_hit(1U, 1U);
    SAG_ASSERT_EQ_U64(got.kind, SAG_REGION_NONE);
    SAG_ASSERT_EQ_I64(got.payload, 0);
}
...
void test_region_empty_query_is_counted(void)
{
    ...
    sag_region_frame_begin();
    before = sag_region_empty_queries();
    (void)sag_region_hit(0U, 0U);
    SAG_ASSERT_EQ_U64(sag_region_empty_queries(), (u64)before + 1U);

    sag_region_add(SAG_REGION_PANE, (Rect){0U, 0U, 4U, 4U}, 0);
    (void)sag_region_hit(0U, 0U);
...
void test_region_capacity_drops_without_growing(void)
{
    ...
    sag_region_frame_begin();
    for (i = 0U; i < (u32)SAG_REGION_MAX + 50U; i++)
        sag_region_add(SAG_REGION_PANE, (Rect){0U, (u16)(i % 24U), 1U, 1U},
                       (i32)i);
    SAG_ASSERT_EQ_U64(sag_region_count(), SAG_REGION_MAX);
    /* The table still answers, from what it did keep. */
    SAG_ASSERT_EQ_U64(sag_region_hit(0U, 0U).kind, SAG_REGION_PANE);
}
...
{
    sag_region_frame_begin();
    sag_region_add(SAG_REGION_PANE, (Rect){5U, 5U, 0U, 10U}, 1);
    sag_region_add(SAG_REGION_PANE, (Rect){5U, 5U, 10U, 0U}, 2);
    SAG_ASSERT_EQ_U64(sag_region_count(), 0U);
}
...
void test_region_tab_payload_sign_convention(void)
{
    ...
    sag_region_frame_begin();
    sag_region_add(SAG_REGION_TAB, (Rect){0U, 0U, 10U, 1U}, 2);
    sag_region_add(SAG_REGION_TAB, (Rect){10U, 0U, 10U, 1U}, -5);

    got = sag_region_hit(1U, 0U);
    SAG_ASSERT(got.payload >= 0);
    SAG_ASSERT_EQ_I64(got.payload, 2);
    got = sag_region_hit(11U, 0U);
    SAG_ASSERT(got.payload < 0);
    SAG_ASSERT_EQ_I64(-got.payload, 5);
}
