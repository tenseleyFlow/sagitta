...
void test_fl_marshal_all_command_rows_resolve_once(void)
{
    ...
    for (i = 0U; i < fl_api_len; i++) {
        if (fl_api[i].cmd != NULL)
            ...
            YEW_ASSERT(fl_api[i].query != NULL);
    }
    yew_cmd_shutdown();
}
...
void test_fl_marshal_receiver_lookup_is_kind_scoped(void)
{
    ...
    mf_open(&f);
    b = fl_h_buf_make(&f.ed, yew_ed_doc(&f.ed));
    d = fl_api_find_receiver(b, "len", 3U);
    YEW_ASSERT_NOT_NULL(d);
    YEW_ASSERT_EQ_STR(d->fl_name, "buf.len");
    YEW_ASSERT_NULL(fl_api_find_receiver(b, "pos", 3U));
    YEW_ASSERT_NULL(fl_api_find_receiver(FL_INT_V(1), "len", 3U));
    mf_close(&f);
}
...
void test_fl_marshal_insert_and_text_preserve_embedded_nul(void)
{
    ...
    mf_open(&f);
    args[0] = fl_h_buf_make(&f.ed, yew_ed_doc(&f.ed));
    args[1] = FL_INT_V(0);
    args[2] = strv(&f.fl.vm, bytes, sizeof(bytes));
    YEW_ASSERT(invoke(&f.fl.vm, "buf.insert", args, 3U, &out));
    YEW_ASSERT_EQ_U64(out.t, FL_NIL);
    YEW_ASSERT(invoke(&f.fl.vm, "buf.text", args, 1U, &out));
    YEW_ASSERT_EQ_U64(out.t, FL_STR);
    text = (FlStr *)out.as.o;
    YEW_ASSERT_EQ_U64(text->len, sizeof(bytes));
    YEW_ASSERT_EQ_MEM(text->b, bytes, sizeof(bytes));
    mf_close(&f);
}
...
void test_fl_marshal_ed_run_accepts_every_key_and_preserves_sarg_len(void)
{
    ...
    YEW_ASSERT(fl_api_ed_run(&f.fl.vm, args, 2U, &out));
    text_args[0] = fl_h_buf_make(&f.ed, yew_ed_doc(&f.ed));
    YEW_ASSERT(invoke(&f.fl.vm, "buf.text", text_args, 1U, &out));
    text = (FlStr *)out.as.o;
    YEW_ASSERT_EQ_U64(text->len, sizeof(bytes));
    YEW_ASSERT_EQ_MEM(text->b, bytes, sizeof(bytes));
    ...
    YEW_ASSERT(invoke(&f.fl.vm, "buf.text", text_args, 1U, &out));
    text = (FlStr *)out.as.o;
    YEW_ASSERT_EQ_U64(text->len, 2U);
    YEW_ASSERT_EQ_MEM(text->b, "xy", 2U);
...
void test_fl_marshal_ed_run_rejects_unknown_key_and_bad_count(void)
{
    ...
    args[1] = FL_OBJ_V(FL_MAP, map);
    YEW_ASSERT(!fl_api_ed_run(&f.fl.vm, args, 2U, &out));
    YEW_ASSERT(error_is(&f.fl.vm, "key"));
    f.fl.vm.err = FL_NIL_V;
    fl_map_clear(map);
    mapv(&f.fl.vm, map, "count", FL_INT_V(0));
    YEW_ASSERT(!fl_api_ed_run(&f.fl.vm, args, 2U, &out));
    YEW_ASSERT(error_is(&f.fl.vm, "type"));
    f.fl.vm.err = FL_NIL_V;
    fl_map_clear(map);
    mapv(&f.fl.vm, map, "range_kind", strv(&f.fl.vm, "span", 4U));
...
void test_fl_marshal_ed_run_status_mapping(void)
{
    ...
    args[1] = FL_OBJ_V(FL_MAP, map);
    YEW_ASSERT(!fl_api_ed_run(&f.fl.vm, args, 2U, &out));
    YEW_ASSERT(error_is(&f.fl.vm, "name"));
    f.fl.vm.err = FL_NIL_V;
    args[0] = strv(&f.fl.vm, "ed.repeat", 9U);
    YEW_ASSERT(!fl_api_ed_run(&f.fl.vm, args, 2U, &out));
    YEW_ASSERT(error_is(&f.fl.vm, "user"));
    f.fl.vm.err = FL_NIL_V;
...
void test_fl_marshal_ed_commands_is_deterministic_registry_order(void)
{
    ...
    bytebuf_init(&a);
    bytebuf_init(&b);
    YEW_ASSERT(fl_api_ed_commands(&f.fl.vm, NULL, 0U, &first));
    fl_gc_protect(&f.fl.vm, first);
    YEW_ASSERT(fl_fmt_repr(&f.fl.vm, &a, first));
    YEW_ASSERT(fl_api_ed_commands(&f.fl.vm, NULL, 0U, &second));
    YEW_ASSERT(fl_fmt_repr(&f.fl.vm, &b, second));
    YEW_ASSERT_EQ_U64(a.len, b.len);
    YEW_ASSERT_EQ_MEM(a.data, b.data, a.len);
    YEW_ASSERT(((FlList *)first.as.o)->n == yew_cmd_count());
    fl_gc_release(&f.fl.vm, 1U);
    bytebuf_free(&b);
    bytebuf_free(&a);
    mf_close(&f);
...
void test_fl_marshal_ed_run_enforces_caps_and_rejects_nul_paths(void)
{
    ...
    YEW_ASSERT(!fl_api_ed_run(&f.fl.vm, args, 2U, &out));
    YEW_ASSERT(error_is(&f.fl.vm, "capability"));
    f.fl.vm.err = FL_NIL_V;
...
void test_fl_marshal_cursor_mutation_targets_the_receiver(void)
{
    ...
    args[1] = FL_INT_V(0);
    {
        ...
        YEW_ASSERT(invoke(&f.fl.vm, "buf.insert", insert, 3U, &out));
    }
    ...
    args[1] = FL_INT_V(3);
    YEW_ASSERT(invoke(&f.fl.vm, "cur.goto", args, 2U, &out));
    YEW_ASSERT_EQ_U64(f.ed.win->cs.curs.data[other].pos.v, other_before);
    YEW_ASSERT_EQ_U64(f.ed.win->cs.curs.data[target].pos.v, 3U);
    mf_close(&f);
}
...
void test_fl_marshal_teardown_blocks_mutation_but_not_queries(void)
{
    ...
    YEW_ASSERT(error_is(&f.fl.vm, "user"));
    f.fl.vm.err = FL_NIL_V;
    YEW_ASSERT(invoke(&f.fl.vm, "buf.len", args, 1U, &out));
