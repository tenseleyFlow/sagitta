...
static void diff_emit_pane(FlEmit *e, const char *key, u32 depth)
{
    ...
    if (depth == 0U) {
        ...
        yew_fl_str(e, "split", (depth & 1U) != 0U ? "h" : "v", 1U);
        yew_fl_int(e, "ratio_permille", 500);
...
static void diff_generated_doc(Bytebuf *raw, u32 serial, u64 *rng)
{
    ...
    yew_fl_emit_init(&e, raw);
    yew_fl_map_open(&e, NULL);
    yew_fl_int(&e, "version", 1);
    yew_fl_str(&e, "writer", "yew", 3U);
    yew_fl_map_open(&e, "workspace");
    yew_fl_str(&e, "path", "/generated", 10U);
    yew_fl_int(&e, "saved_at", (i64)serial);
    yew_fl_map_close(&e);
    yew_fl_map_open(&e, "options");
    yew_fl_map_close(&e);
    yew_fl_list_open(&e, "groups");
    yew_fl_map_open(&e, NULL);
    yew_fl_int(&e, "id", 7);
    yew_fl_str(&e, "label", "generated", 9U);
    yew_fl_str(&e, "dir_path", "/generated", 10U);
    yew_fl_nil(&e, "last_active_member");
    yew_fl_map_close(&e);
    yew_fl_list_close(&e);
    yew_fl_list_open(&e, "tabs");
    for (i = 0U; i < ntabs; i++) {
        const char *path = paths[diff_rng(rng) % YEW_ARRAY_LEN(paths)];
        u32 ncursors = 1U + (u32)(diff_rng(rng) % 8U);
        u32 c;

        yew_fl_map_open(&e, NULL);
        yew_fl_int(&e, "id", (i64)i + 1);
        yew_fl_str(&e, "path", path, (u64)strlen(path));
        yew_fl_int(&e, "group", (i % 3U) == 0U ? 7 : 0);
        yew_fl_int(&e, "group_ordinal", (i64)i);
        yew_fl_bool(&e, "deferred", i != 0U);
        yew_fl_int(&e, "focus", 0);
        diff_emit_pane(&e, "panes", (u32)(diff_rng(rng) % 9U));
        yew_fl_list_open(&e, "wins");
        yew_fl_map_open(&e, NULL);
        yew_fl_list_open(&e, "cursors");
        for (c = 0U; c < ncursors; c++) {
            yew_fl_map_open(&e, NULL);
            yew_fl_int(&e, "pos", (i64)(diff_rng(rng) % 100000U));
            yew_fl_int(&e, "anchor", (i64)(diff_rng(rng) % 100000U));
            yew_fl_int(&e, "goal", c == 0U ? -1 : (i64)c);
            yew_fl_map_close(&e);
        }
        yew_fl_list_close(&e);
        yew_fl_int(&e, "primary", (i64)(ncursors - 1U));
        yew_fl_map_close(&e);
        yew_fl_list_close(&e);
        yew_fl_map_close(&e);
    }
    yew_fl_list_close(&e);
    yew_fl_int(&e, "active_tab", ntabs == 0U ? 0 : 1);
    yew_fl_list_open(&e, "files");
    yew_fl_list_close(&e);
    yew_fl_map_close(&e);
    yew_fl_emit_done(&e);
}
...
void test_state_diff_generated_500_matrix(void)
{
    ...
    for (i = 0U; i < 500U; i++) {
        ...
        old_lit = yew_fl_parse(&old_arena, raw.data, raw.len, &old_err);
        new_lit = yew_fl_parse_fletch(&new_arena, raw.data, raw.len,
                                      &new_err);
        YEW_ASSERT_NOT_NULL(old_lit);
        YEW_ASSERT_NOT_NULL(new_lit);
        diff_reemit_lit(old_lit, &old_out);
        diff_reemit_lit(new_lit, &new_out);
        YEW_ASSERT_EQ_U64(old_out.len, raw.len);
        YEW_ASSERT_EQ_MEM(old_out.data, raw.data, raw.len);
        YEW_ASSERT_EQ_U64(new_out.len, raw.len);
        YEW_ASSERT_EQ_MEM(new_out.data, raw.data, raw.len);
        ...
        YEW_ASSERT_EQ_U64(value_out.len, raw.len);
        YEW_ASSERT_EQ_MEM(value_out.data, raw.data, raw.len);
        fl_vm_free(&vm);
        interner_free(&in);
