...
/* ------------------------------------------------------------------ */
/* Sprint 57.11 §4: ed.edit.paste                                     */
/* ------------------------------------------------------------------ */

static void edit_set_unnamed(Ed *ed, const char *text, RegType type)
{
    RegVal v;

    yew_regval_init(&v);
    bytebuf_append(&v.bytes, (const u8 *)text, strlen(text));
    v.type = (u8)type;
    yew_reg_set(&ed->regs, (u8)'"', &v);
    yew_regval_free(&v);
}

void test_edit_paste_char_lands_inline_after_the_cursor(void)
{
    static const u8 text[] = "ace\n";
    Ed ed;

    edit_fixture(&ed, text, sizeof(text) - 1U, YEW_EOL_LF);
    ed.regs.clipboard_sync = YEW_CLIP_SYNC_OFF;
    edit_set_unnamed(&ed, "bd", YEW_REG_CHARWISE);
    edit_place(&ed, 0U);
    YEW_ASSERT_EQ_U64(edit_invoke(&ed, "ed.edit.paste", 1U, false, NULL,
                                  0U), YEW_CMD_OK);
    edit_assert_text(&ed, (const u8 *)"abdce\n", 6U);
    /* The caret sits on the last pasted grapheme, as every other paste
     * route leaves it. */
    YEW_ASSERT_EQ_U64(yew_ed_cursor(&ed)->pos.v, 2U);
    /* One undo node takes the whole paste back. */
    YEW_ASSERT_EQ_U64(edit_invoke(&ed, "ed.edit.undo", 1U, false, NULL,
                                  0U), YEW_CMD_OK);
    edit_assert_text(&ed, text, sizeof(text) - 1U);
    yew_ed_free(&ed);
}

void test_edit_paste_line_opens_a_new_line_below(void)
{
    ...
    edit_fixture(&ed, text, sizeof(text) - 1U, YEW_EOL_LF);
    ed.regs.clipboard_sync = YEW_CLIP_SYNC_OFF;
    edit_set_unnamed(&ed, "zz\n", YEW_REG_LINEWISE);
    edit_place(&ed, 1U);
    YEW_ASSERT_EQ_U64(edit_invoke(&ed, "ed.edit.paste", 1U, false, NULL,
                                  0U), YEW_CMD_OK);
    edit_assert_text(&ed, (const u8 *)"aa\nzz\nbb\n", 9U);
...
void test_edit_paste_refuses_on_a_read_only_buffer(void)
{
    ...
    YEW_ASSERT_EQ_U64(edit_invoke(&ed, "ed.edit.paste", 1U, false, NULL,
                                  0U), YEW_CMD_ERR_STATE);
    edit_assert_text(&ed, text, sizeof(text) - 1U);
    YEW_ASSERT_EQ_U64(ed.msg.sev, YEW_MSG_ERROR);
    YEW_ASSERT_EQ_STR(ed.msg.text, "buffer is read-only");
    yew_ed_free(&ed);
}
...
/* Invariant 9: the `Paste` row has a key.  `p` was free in L. */
void test_edit_paste_is_bound_to_p_in_line_mode(void)
{
    ...
    edit_place(&ed, 0U);
    YEW_ASSERT_EQ_U64(ed.mode, YEW_MODE_L);
    yew_ed_handle_key(&ed, edit_key((u32)'p'), 0);
    YEW_ASSERT_EQ_U64(ed.last_status, YEW_CMD_OK);
    edit_assert_text(&ed, (const u8 *)"abdce\n", 6U);
    yew_ed_free(&ed);
}
