                               ...
                               char *why, size_t why_cap)
{
    ...
    while (pos < len) {
        ...
        d->off[d->len] = pos;
        d->state[d->len] = state;
        consumed = sag_utf8_decode(data + pos, len - pos, &d->cp[d->len]);
        if (consumed == 0U || consumed > len - pos)
            return fail(why, why_cap, "decoder did not advance", pos);
        d->boundary[d->len] = sag_gb_boundary(&state, d->cp[d->len]);
        pos += consumed;
        d->len++;
    }
    d->off[d->len] = len;
...
static bool check_restart(const Decoded *d, char *why, size_t why_cap)
{
    ...
    for (start = 0U; start < d->len; start++) {
        ...
        for (i = start; i < d->len; i++) {
            bool boundary = sag_gb_boundary(&state, d->cp[i]);
                             ...
                             char *why, size_t why_cap)
{
    ...
    while (pos < len) {
        size_t next = sag_gb_next_bytes(data, len, pos);
        SagCluster cluster;
        ...
        if (!sag_utf8_is_boundary(data, len, pos) ||
            !sag_utf8_is_boundary(data, len, next))
            return fail(why, why_cap,
                        "grapheme boundary split a codepoint", pos);
        if (!sag_cluster_next(data, len, &cluster_pos, &cluster) ||
            cluster_pos != next || cluster.off != pos ||
            cluster.len != next - pos)
            return fail(why, why_cap,
                        "cluster iterator disagrees with segmenter", pos);
        if (cluster.cells != SAG_CLUSTER_TAB && cluster.cells > 4U)
            return fail(why, why_cap, "cluster width outside 0..4", pos);
        {
            int width = sag_cluster_width(data + pos, next - pos);
            ...
            if ((width < 0 && !(next - pos == 1U && data[pos] == '\t')) ||
                width > 4)
                return fail(why, why_cap,
                            "sag_cluster_width outside 0..4", pos);
        }
        pos = next;
    }
    if (pos != len)
                            ...
                            char *why, size_t why_cap)
{
    ...
    while (p < len) {
        size_t next = sag_gb_next_bytes(data, len, p);
        size_t prev = sag_gb_prev_bytes(data, len, next);

        if (prev > next || sag_gb_next_bytes(data, len, prev) != next)
            return fail(why, why_cap,
...
static bool check_clipping(const u8 *data, size_t len,
                           char *why, size_t why_cap)
{
    int total = sag_str_width(data, len, 4U);
    int max_limit;
    int max_cells;
    ...
    for (max_cells = 0; max_cells <= max_limit; max_cells++) {
        int cells = -1;
        size_t kept = sag_str_clip(data, len, max_cells, &cells);

        if (kept > len || cells < 0 || cells > max_cells)
            return fail(why, why_cap, "clip exceeded cell or byte bound", kept);
        if (!sag_utf8_is_boundary(data, len, kept))
            return fail(why, why_cap, "clip split a codepoint", kept);
        if (kept != 0U) {
            size_t prev = sag_gb_prev_bytes(data, len, kept);

            if (sag_gb_next_bytes(data, len, prev) != kept)
                           ...
                           char *why, size_t why_cap)
{
    ...
    if (!sag_fuzz_check_utf8(data, len, why, why_cap))
        return false;
...
{
    return sag_fuzz_main(argc, argv, "fuzz_grapheme",
