/*
 * Sprint 20 DoD 7: yew_re_compile over arbitrary bytes.
 *
                        ...
                        char *why, size_t why_cap)
{
    ...
    err.off = 0xFFFFFFFFU;
    re = yew_re_compile(&arena, (const char *)data, len, flags, &err);
    if (re == NULL) {
        if (err.msg == NULL) {
    ...
    if (yew_re_group_count(re) == 0U ||
        yew_re_group_count(re) > YEW_RE_MAX_GROUPS) {
        (void)snprintf(why, why_cap, "compiled with %u groups",
                       (unsigned)yew_re_group_count(re));
        arena_free_all(&arena);
        return false;
    }
    {
        static const u8 probe[] = "abc 123 \xE6\xBC\xA2\xF0\x9F\x98\x80";
        YewReInput in = yew_re_input_bytes(probe, sizeof(probe) - 1U);
        ...
        (void)yew_re_test(re, &in, BYTEOFF(0U));
    }
    arena_free_all(&arena);
    return true;
}
...
{
    return yew_fuzz_main(argc, argv, "fuzz_re_compile", NULL,
                         check_re_compile);
}
