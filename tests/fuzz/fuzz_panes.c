...
static bool focus_is_live(Ed *ed)
{
    ...
    collect(ed->pane_root, leaves, &n, YEW_ARRAY_LEN(leaves));
    for (i = 0U; i < n; i++) {
                        ...
                        size_t why_cap)
{
    ...
    for (op = 0U; op < ops; op++) {
        ...
        case 0:
        case 1: {
            Pane *nu = yew_pane_split(&ed, pick,
                                      rng_below(&rng, 2U) != 0U
                                          ? YEW_SPLIT_V
                                          : YEW_SPLIT_H);
        ...
        case 2:
            if (pick->parent != NULL) {
                ...
                if (ed.focus == pick)
                    ed.focus = NULL;
                (void)yew_pane_close(&ed, pick);
                if (ed.focus == NULL)
                    ed.focus = yew_pane_first_leaf(parent);
            }
            break;
        case 3: {
            Pane *split = yew_pane_ancestor_split(pick, YEW_SPLIT_H);

            if (split == NULL)
                split = yew_pane_ancestor_split(pick, YEW_SPLIT_V);
            if (split != NULL)
                (void)yew_pane_resize(split,
                                      (i32)rng_below(&rng, 9U) - 4);
            break;
        }
        case 4: {
            Pane *to = yew_pane_dir(ed.pane_root, ed.focus,
                                    (YewDir)rng_below(&rng, 4U));
         ...
         * move a single cell. */
        {
            Pane *before[YEW_PANE_MAX_LEAVES * 2];
            Rect snap[YEW_PANE_MAX_LEAVES * 2];
            u32 m = 0U;
            u32 i;

            collect(ed.pane_root, before, &m, YEW_ARRAY_LEN(before));
            for (i = 0U; i < m; i++)
...
{
    return yew_fuzz_main(argc, argv, "fuzz_panes", NULL, run_session);
}
